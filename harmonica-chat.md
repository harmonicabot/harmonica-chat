...
Compare the version in the first line of the response (`<!-- harmonica-chat vX.Y.Z -->`) against `v2.11.0` (this file's version). If the remote version is newer, inform the user before proceeding:

> **Update available:** harmonica-chat `v{remote}` is out (you have `v2.11.0`). Run this to update:
> ```
