...
1. **Resolve the session.** Per SKILL.md § Session reference resolution, the input may already be a UUID (from a pasted URL or bare ID). If it looks UUID-shaped, call `get_session` directly. Otherwise call `search_sessions` with the input as the query.
...
4. (If you took the search path) Call `get_session` with the matched session ID to get metadata.
